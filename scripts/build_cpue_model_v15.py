#!/usr/bin/env python3
"""
CPUE Model v15 — Two-Model Architecture + Enhanced Temporal Features
=====================================================================
...
Usage:
    python scripts/build_cpue_model_v15.py [--workspace /workspace/castline]
"""
...
# GPU / CatBoost config
# ---------------------------------------------------------------------------
_USE_GPU = bool(os.environ.get("USE_GPU"))
_CB_TASK = "GPU" if _USE_GPU else "CPU"
# GPU mode uses Bayesian bootstrap by default which doesn't support subsample;
# switch to Bernoulli when subsample is needed.
_CB_BOOTSTRAP = {"bootstrap_type": "Bernoulli"} if _USE_GPU else {}
...
DATASET_CANDIDATES = [
    BASE_DIR / "castline" / "validation" / "data" / "assembled" / "validation_dataset_v17.csv",
    BASE_DIR / "data" / "assembled" / "validation_dataset_v17.csv",
    BASE_DIR / "validation_dataset_v17.csv",
]
...
def prune_features(df, features, target_col="log_cpue", label=""):
    ...
    m_full = CatBoostRegressor(
        ...
        task_type=_CB_TASK, thread_count=-1, **_CB_BOOTSTRAP,
    )
...
def train_stacked_model(df, features, label, best_cfg, target_col="log_cpue"):
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(df, y, groups)):
        ...
        cb = CatBoostRegressor(
            ...
            random_seed=SEED, task_type=_CB_TASK, verbose=0, early_stopping_rounds=200,
            thread_count=-1, **_CB_BOOTSTRAP,
        )
...
def train_final_models(df, features, best_cfg, target_col="log_cpue"):
    ...
    cb = CatBoostRegressor(
        ...
        l2_leaf_reg=best_cfg["l2"], subsample=best_cfg["sub"],
        random_seed=SEED, task_type=_CB_TASK, verbose=0, thread_count=-1, **_CB_BOOTSTRAP,
    )
...
def hp_search(df, features, label=""):
    ...
    for cfg in HP_CONFIGS:
        m = CatBoostRegressor(
            ...
            random_seed=SEED, task_type=_CB_TASK, verbose=0, early_stopping_rounds=150,
            thread_count=-1, **_CB_BOOTSTRAP,
        )
...
def save_artifacts(df, seen_features, unseen_features, seen_cfg, unseen_cfg,
    ...
    metadata = {
        ...
        "dataset": "validation_dataset_v17.csv + enhanced temporal features",
        "n_rows": len(df),
