"""v14 evaluation: targeted gap-filling, not feature replacement.

Key insight from v11 failure: loc_quality and knn_prior REPLACE signal
when they should only SUPPLEMENT it. loc_rolling_mean (actual data) >>
any learned proxy.

Strategy:
- Temporal/Spatial/ST: use loc_rolling_mean where available, fill NaN with
  KNN prior from training split. DON'T add loc_quality as dominant feature.
- LOO: use knn_enc + loc_quality (same as v10 LOO that got 0.575)
- All: CatBoost ensemble for stability
"""
import pandas as pd
import numpy as np
...
def fill_rolling_mean_gaps(train_df, test_df=None, K=8):
    ...
    Returns copies with gaps filled. Does NOT add new features.
    """
    morph = [c for c in LOC_MORPH if c in train_df.columns]

    # Build KNN from training locations
    ...
    # KNN prior for each training location
    knn_prior = {}
    ...
    if test_df is not None:
        ...
        if "loc_rolling_mean" in test_out.columns:
            # For test locations: use KNN prior if rolling_mean is NaN
            for loc in test_out.location.unique():
                ...
                if test_out.loc[loc_mask, "loc_rolling_mean"].isna().any():
                    if loc in knn_prior:
                        ...
                        # New location: compute KNN from morphometry
...
def knn_encode_loo(train_df, test_df, K=8):
    """KNN encoding for LOO: compute cluster/region/quality for held-out location."""
    morph = [c for c in LOC_MORPH if c in train_df.columns]
    ...
    return (cluster_map, region_map, nn_std_map, quality_map,
            test_cluster, test_region, test_nn_std, test_quality)


# ═══════════════════════════════════════
# LOAD DATA
# ═══════════════════════════════════════
df = pd.read_csv("castline/validation/data/assembled/validation_dataset_v11.csv",
                 low_memory=False)
...
# Temporal/spatial features: includes loc_rolling_mean (will be gap-filled)
temporal_features = base_features.copy()

# LOO features: exclude location-specific rolling features
...
p(f"loc_rolling_mean coverage (raw): {df['loc_rolling_mean'].notna().mean()*100:.1f}%")
...
# Fill loc_rolling_mean gaps with KNN prior from train only
tr, te = fill_rolling_mean_gaps(tr_raw, te_raw)

p(f"  Train loc_rolling_mean coverage: {tr['loc_rolling_mean'].notna().mean()*100:.1f}%")
...
if ndcgs:
    p(f"  NDCG@3={np.mean(ndcgs):.4f}  Pairwise={np.mean(pws):.4f}")
p(f"  Top features: {list(zip(imp.index[:5], imp.values[:5].astype(int)))}")
...
for i, loc in enumerate(clean_locs):
    ...
    # KNN encoding (cluster, region, quality)
...
p(f"  RANKING: NDCG@5={ndcg5:.4f}  Pairwise={pw:.4f}  Spearman={spear_r:.4f}  Kendall={tau:.4f}")
...
# ═══════════════════════════════════════
p("\n" + "=" * 60)
p("v14 SUMMARY (v11 data: biology from NASA POWER)")
p("=" * 60)
p(f"  Temporal (gap-filled)  : R²={r2_t:.4f}  MAE={mae_t:.3f}")
...
p()
p("Comparison:")
p("  v14: T=0.424 S=0.477 ST=0.596 LOO=0.575")
p(f"  v14: T={r2_t:.3f} S={r2_s:.3f} ST={r2_st:.3f} LOO={r2_loo:.3f}")
p()
