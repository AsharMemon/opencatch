"""v15 evaluation with GeoCLIP location embeddings.

Strategy: Merge PCA-reduced GeoCLIP embeddings (32 or 64 dim) with v15 dataset,
then run the same pruned feature + multi-library ensemble pipeline.

GeoCLIP embeddings encode satellite/geographic information about each location
into a dense vector, potentially helping the model generalize to unseen locations.
"""
import pandas as pd
import numpy as np
...
# Load GeoCLIP embeddings and merge
for pca_dim in [32, 64]:
    emb_path = f"castline/validation/data/raw/location_embeddings_geoclip_pca{pca_dim}.csv"
    emb = pd.read_csv(emb_path)
    p(f"GeoCLIP PCA-{pca_dim}: {len(emb)} locations, {emb.shape[1]} columns")

    # Round lat/lon for merge (avoid floating point mismatches)
    df["_lat_r"] = df["lat"].round(4)
    df["_lon_r"] = df["lon"].round(4)
    emb["_lat_r"] = emb["lat"].round(4)
    emb["_lon_r"] = emb["lon"].round(4)

    geoclip_cols = [c for c in emb.columns if c.startswith("geoclip_")]

    # Merge
    merge_cols = ["_lat_r", "_lon_r"] + geoclip_cols
    df_merged = df.drop(columns=[c for c in geoclip_cols if c in df.columns], errors="ignore")
    df_merged = df_merged.merge(emb[merge_cols], on=["_lat_r", "_lon_r"], how="left")

    matched = df_merged[geoclip_cols[0]].notna().sum()
    p(f"  Matched: {matched}/{len(df_merged)} rows ({matched/len(df_merged)*100:.1f}%)")
...
df.drop(columns=["_lat_r", "_lon_r"], errors="ignore", inplace=True)


# ═══════════════════════════════════════
# EVALUATE BOTH PCA DIMS
# ═══════════════════════════════════════
for label, dfe, pca_dim in [("PCA-32", df32, 32), ("PCA-64", df64, 64)]:
    p(f"\n{'='*60}")
    p(f"EVALUATING GeoCLIP {label}")
    p(f"{'='*60}")

    geoclip_cols = [c for c in dfe.columns if c.startswith("geoclip_")]

    all_cols = set(dfe.columns)
    ...
    all_features = [f for f in all_features if dfe[f].dtype in ["float64", "int64", "float32", "int32"]]

    temporal_all = all_features.copy()
    loo_all = [f for f in all_features if f not in LOO_EXCLUDE]

    p(f"Features: {len(temporal_all)} total ({len(geoclip_cols)} GeoCLIP)")

    # ── Feature importance pruning ──
    dfe_s = dfe.sort_values("date")
    split = int(len(dfe_s) * 0.8)
    tr_raw, te_raw = dfe_s.iloc[:split].copy(), dfe_s.iloc[split:].copy()
    ...
    # Count GeoCLIP features that survived pruning
    gc_survived = [f for f in temporal_pruned if f.startswith("geoclip_")]
    p(f"GeoCLIP features in pruned set: {len(gc_survived)}/{len(geoclip_cols)}")
    if gc_survived:
        for f in gc_survived[:10]:
            p(f"  {f}: importance={imp_full[f]:.3f}%")
        if len(gc_survived) > 10:
            p(f"  ... and {len(gc_survived)-10} more")
    ...
    r2_seen = r2_score(te_seen[TARGET], ensemble_pred[te.location.isin(seen_locs).values]) if len(te_seen) > 0 else 0
    r2_unseen = r2_score(te_unseen[TARGET], ensemble_pred[~te.location.isin(seen_locs).values]) if len(te_unseen) > 0 else 0

    p(f"  CatBoost:  R²={r2_cb:.4f}")
    p(f"  Ensemble:  R²={r2_ens:.4f}")
    p(f"  Seen: R²={r2_seen:.4f}  Unseen: R²={r2_unseen:.4f}")
    p(f"  [{time.time()-t0:.0f}s]")

    # ── SPATIAL ──
    p(f"\n--- SPATIAL ---")
    t0 = time.time()
    dfe["_region"] = pd.cut(dfe.lat, bins=5, labels=["S", "SM", "M", "MN", "N"])
    rr = {}
    for region in dfe["_region"].unique():
        te_mask = dfe._region == region
        tr_r, te_r = fill_rolling_mean_gaps(dfe[~te_mask], dfe[te_mask])
    ...
    p(f"\n--- SPATIOTEMPORAL ---")
    t0 = time.time()
    dfe["_block"] = dfe["_region"].astype(str) + "_" + (dfe.year // 3).astype(str)
    gkf = GroupKFold(n_splits=5)
    fold_r2s = []
    for tr_idx, te_idx in gkf.split(dfe, groups=dfe["_block"]):
        tr_r, te_r = fill_rolling_mean_gaps(dfe.iloc[tr_idx], dfe.iloc[te_idx])
        preds = []
        for cfg in cb_configs[:2]:
            m = CatBoostRegressor(**cfg)
            m.fit(tr_r[temporal_pruned], tr_r[TARGET])
    ...
    loc_counts = dfe.location.value_counts()
    loc_std = dfe.groupby("location")[TARGET].std()
    ...
    morph = [c for c in LOC_MORPH if c in dfe.columns]
    # Include GeoCLIP in morph for LOO KNN
    morph_plus_gc = morph + gc_survived[:8]  # Top 8 GeoCLIP dims for KNN

    all_true, all_pred = [], []
    per_loc = {}

    for i, loc in enumerate(clean_locs):
        mask = dfe.location == loc
        train, test = dfe[~mask].copy(), dfe[mask].copy()
    ...
    # ── SUMMARY for this PCA dim ──
    p(f"\n{'='*60}")
    p(f"GeoCLIP {label} SUMMARY")
    p(f"{'='*60}")
    p(f"  Features: {best_n} pruned ({len(gc_survived)} GeoCLIP survived)")
