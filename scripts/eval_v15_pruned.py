"""v15 evaluation with aggressive feature pruning.

Strategy: Train a model with ALL features, get importance, keep only
the TOP N features, then retrain with fewer features for better
generalization. Also try multi-library ensemble.
"""
import pandas as pd
import numpy as np
...
p(f"All features: {len(temporal_all)} temporal, {len(loo_all)} LOO")
...
# ═══════════════════════════════════════
p("\n" + "=" * 60)
p("STEP 1: Feature importance-based pruning")
p("=" * 60)
...
# Test different top-N cutoffs
for top_n in [50, 75, 100, 150]:
    ...
    p(f"  Top-{top_n:3d}: R²={r2:.4f}")

p(f"  All-{len(temporal_all):3d}: R²={r2_score(te[TARGET], np.clip(m_full.predict(te[temporal_all]), 1, 25)):.4f}")

# Find optimal N
best_n, best_r2 = 0, -np.inf
for top_n in range(30, 180, 10):
    top_feats = imp_full.head(top_n).index.tolist()
    m = CatBoostRegressor(iterations=1000, depth=6, learning_rate=0.05,
                          l2_leaf_reg=3, verbose=0, random_seed=42, subsample=0.85)
    m.fit(tr[top_feats], tr[TARGET])
...
p(f"  Pruned temporal features: {len(temporal_pruned)}")
p(f"  Pruned LOO features: {len(loo_pruned)}")

# Show which new features survived
new_in_pruned = [f for f in temporal_pruned if "lag_" in f or "gdd_" in f or
                 "photo" in f or "lagos_" in f or "thermal" in f or "spawn" in f or
                 "stratification" in f or "seasonal" in f or "pressure_" in f]
p(f"  New features in pruned set ({len(new_in_pruned)}):")
for f in new_in_pruned:
    p(f"    {f:40s} imp={imp_full.get(f, 0):.2f}%")


# ═══════════════════════════════════════
# STEP 2: Multi-library ensemble on pruned features
# ═══════════════════════════════════════
p("\n" + "=" * 60)
p("TEMPORAL with pruned features + multi-library ensemble")
p("=" * 60)
t0 = time.time()
...
r2_xgb = r2_score(te[TARGET], np.clip(xgb_pred, 1, 25))
r2_lgb = r2_score(te[TARGET], np.clip(lgb_pred, 1, 25))
r2_ens = r2_score(te[TARGET], ensemble_pred)

p(f"  CatBoost:  R²={r2_cb:.4f}")
p(f"  XGBoost:   R²={r2_xgb:.4f}")
p(f"  LightGBM:  R²={r2_lgb:.4f}")
p(f"  Ensemble:  R²={r2_ens:.4f}")
p(f"  [{time.time()-t0:.0f}s]")

r2_t = max(r2_cb, r2_ens)  # Use best
r2_seen = r2_score(te_seen[TARGET], ensemble_pred[te.location.isin(seen_locs).values])
r2_unseen = r2_score(te_unseen[TARGET], ensemble_pred[~te.location.isin(seen_locs).values])
p(f"  Seen: R²={r2_seen:.4f}  Unseen: R²={r2_unseen:.4f}")


# ═══════════════════════════════════════
# SPATIAL with pruned features
# ═══════════════════════════════════════
p("\n" + "=" * 60)
p("SPATIAL with pruned features")
p("=" * 60)
t0 = time.time()
...
# ═══════════════════════════════════════
# LOO with pruned features
# ═══════════════════════════════════════
p("\n" + "=" * 60)
p("LOO with pruned features + multi-model")
p("=" * 60)
t0 = time.time()
...
for i, loc in enumerate(clean_locs):
    ...
    # Multi-library ensemble for LOO
    preds = []
    for cfg in cb_configs[:2]:
        m = CatBoostRegressor(**cfg)
...
# ═══════════════════════════════════════
p("\n" + "=" * 60)
p("v15 PRUNED SUMMARY")
p("=" * 60)
p(f"  Features used: {best_n} (pruned from {len(temporal_all)})")
...
p(f"  v15 pruned: T={r2_t:.3f} S={r2_s:.3f} ST={r2_st:.3f} LOO={r2_loo:.3f}")
